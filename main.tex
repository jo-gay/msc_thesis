...
%Modern microscopy, by utilizing combinations of different imaging techniques, enables capturing of overlapping fields of view of the same specimen in micro- and nano-scales, providing very information rich data. %from original thesis proposal, no citations
...
%They use the exponential of the distance measure $D$ divided by a local variance measure $V$ (equal to the average distance metric between the current pixel and its four immediate neighbours) and normalized with a constant $n$, calculated for some set of nearby pixels $r \in R$, as a descriptive vector for each pixel:
...
%The CNN described above treats each output from the first layer as a separate input to the second layer, leading to $L_1L_2$ feature map outputs for each image after layer 2. 
